...
public:
    ...
    Vector3d getVertex(int i) { return vertices[i];}

    //set Min indexes in XYZ direction
    void setMins(Vector3d m) { mins = m;}

    //set Max indexes in XYZ direction
    ...
    // Vertex number
    inline vector<Vector3d> getOriginalVertices() const {
        return originalVertices;
    }

    inline void setOriginalVertices(vector<Vector3d> originalVertices) {
...
protected:
    ...
    // Global color (paint the whole object with the same color using this value)
